# 设数组 a 的长度为 n，元素为 a[0],a[1],⋯,a[n−1]，数组元素的最大值为 max(a)。
# 我们选取进制 k 满足 k > max(a)，将数组 a 看成一个 n 位的 k 进制整数.

# Must using rolling-hash, so compute hash is O(1)
# But the hash maybe too big so we need to mod

# Sample problem 1923. Longest Common Subpath
